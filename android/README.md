...
# Base environnement
        ...
        cd italo/utils/build_scripts
        docker build -f android32.Dockerfile -t italo-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t italo-gui-android .
        docker create -it --name italo-gui-android italo-gui-android bash

# Get the apk

        docker cp italo-gui-android:/opt/android/italo-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/italo-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
