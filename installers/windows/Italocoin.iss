...
LicenseFile=LICENSE
AppPublisher=The Italocoin Developer Community
AppPublisherURL=https://getitalocoin.org
...
; As of version 0.12.3.0, the Italocoin .exe files do not carry version info anyway in their .exe headers.
...
Source: "bin\italocoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "bin\italocoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\italocoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\italocoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
...
begin
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\italocoin-project\italocoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Italocoin default location
...
Root: HKCU; Subkey: "Software\italocoin-project\italocoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};
