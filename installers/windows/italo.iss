; Italo Beryllium Bullet GUI Wallet Installer for Windows
...
; As of version 0.13.0.4, the Italo .exe files do not carry version info anyway in their .exe headers.
...
Source: "bin\italo-GUI-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Italo GUI wallet log file
...
Source: "italo-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify
...
; Source: "bin\italo-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
var s: String;
begin
  s := ExpandConstant('{app}\italo-wallet-gui.log');
  if Pos(' ', s) > 0 then begin
    // Quotes needed for filename with blanks
    s := '"' + s + '"';
  end;
...
Name: "{group}\GUI Wallet"; Filename: "{app}\italo-wallet-gui.exe"; Parameters: {code:WalletFlags}
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\italo-GUI-guide.pdf"; IconFilename: "{app}\italo-wallet-gui.exe"
...
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\italo-wallet-gui.log"
...
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\italo-wallet-gui.exe"; Parameters: {code:WalletFlags}; Tasks: desktopicon
