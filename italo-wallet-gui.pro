...
# cleaning "auto-generated" bititalo directory on "make distclean"
...
# translation stuff
TRANSLATIONS = $$files($$PWD/translations/italo-core_*.ts)
...
win32 {
    deploy.commands += windeployqt $$sprintf("%1/%2/%3.exe", $$OUT_PWD, $$DESTDIR, $$TARGET) -debug -no-translations -qmldir=$$PWD
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libitalo-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
