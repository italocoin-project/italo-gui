...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = italoAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date, _nettype){
    // time of italo birth 2014-04-18 10:49:53 (1397818193)
    var italoBirthTime = _nettype == "Mainnet" ? 1397818193 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    ...
    var secondsPerBlock;
    // before italo's birth
    if (requestedTime < italoBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < italoBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > italoBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - italoBirthTime)/secondsPerBlockV1);
